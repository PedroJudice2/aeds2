...
void game_print(Game *game)
{
    ...
    if (game->count_genres - 1 == 0)
    {
        chomp(game->genres[0]);
    }
    for (int i = 0; i < game->count_genres - 1; i++)
    {
        printf("%s, ", game->genres[i]);
    }
    printf("%s] \n", game->genres[game->count_genres - 1]);
}
...
int getMaior(void)
{
    int maior = (int)games[0].upvotes;

    for (int i = 0; i < n; i++)
    {
        if (maior < games[i].upvotes)
        {
            maior = (int)games[i].upvotes;
        }
    }
    return maior;
}
